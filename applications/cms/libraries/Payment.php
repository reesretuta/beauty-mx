...
/**
	...
	function capture_transaction($data=array())
	{
		...
		*/
		...
		{
			return $parsedresponse->messages->message;
		}
	...
	/**
	 * @author Shalltell Uduojie
	 * @desc This is used to void transactions
	 * @param $profile_id
	 * @param $payment_id
	 * @param $trans_id
	 * 
	...
	function void($profile_id, $payment_id, $trans_id)
	{
		$content=
		"<?xml version=\"1.0\" encoding=\"utf-8\"?>".
		"<createCustomerProfileTransactionRequest xmlns=\"AnetApi/xml/v1/schema/AnetApiSchema.xsd\">".
		...
		$response = $this->send_xml_request($content);
		$parsedresponse = $this->parse_api_response($response);
		printer($parsedresponse);
		
		//@TODO: add response data for void
		return 1;
	}
	
	
	/**
	 * @author Shalltell Uduojie - La Visual
	 * @desc This is used to refund transactions..
	 * @param  $refund_amount
	 * @param  $profile_id
	 * @param  $payment_id
	 * @param  $trans_id
	 */
	function refund($refund_amount,$profile_id, $payment_id, $trans_id)
	{
		$content=
		"<?xml version=\"1.0\" encoding=\"utf-8\"?>".
		"<createCustomerProfileTransactionRequest xmlns=\"AnetApi/xml/v1/schema/AnetApiSchema.xsd\">".
		...
		$response = $this->send_xml_request($content);
		$parsedresponse = $this->parse_api_response($response);
		//@TODO: add response data for refund
		
		if($parsedresponse->messages->resultCode=='Error')
		{
			
			return $parsedresponse->messages->text;
		}
		else
		{
			return TRUE;
	...
	function create_customer_profile($data=array())
	{
		$content =
		"<?xml version=\"1.0\" encoding=\"utf-8\"?>" .
		...
		$this->MerchantAuthenticationBlock().
		"<profile>".
		"<merchantCustomerId>".$data['user_id']."</merchantCustomerId>". // Your own identifier for the customer.
