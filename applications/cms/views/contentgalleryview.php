								...
								<? foreach($rows as $col=>$row):?>
									<? if(!in_array($col, $column_rules)):?><? //we have rules where we filter. most likely, id will always be avoided?>
										...
										<? elseif(in_array($col, $_paths=unserialize(UPLOAD_PATHS))):?>
											<!-- rollover icons -->
											<div class="rollovers">
												<a class="trash" href="javascript:;" onclick="trash('<?=base_url().'do_trash/'.$table_name.'/'.$rowid?>')" class="trash">remove</a>
												<a href="#" class="view"></a>
											...
											<? if($row):?>
												...
												<a class="noborder noborderh fancybox" rel="group" href="<?=$row?>"><img rel="<?=str_replace('/cms/','',base_url()).$row?>" alt="<?=str_replace('/cms/','',base_url()).$row?>" style="max-width: 135px; max-height: 135px; width: auto; height: auto; display: block" src="http://jafra-mx.s3-website-us-west-1.amazonaws.com<?=$row?>"/></a>
											<? else:?>
