									...
									<? if(!in_array($col, $column_rules)):?><? //we have rules where we filter. most likely, id will always be avoided?>
										...
										<? elseif(in_array($col, $_paths=unserialize(UPLOAD_PATHS))):?>
											...
											<? if($row):?>
												...
												<a class="noborder noborderh fancybox" rel="group" href="http://jafra-mx.s3-website-us-west-1.amazonaws.com<?=$row?>"><img rel="http://jafra-mx.s3-website-us-west-1.amazonaws.com<?=$row?>" alt="http://jafra-mx.s3-website-us-west-1.amazonaws.com<?=$row?>" style="max-width: 135px; max-height: 135px; width: auto; height: auto; display: block" src="http://jafra-mx.s3-website-us-west-1.amazonaws.com<?=$row?>"/></a>
											<? else:?>
