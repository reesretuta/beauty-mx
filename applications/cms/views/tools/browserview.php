		...
		<? if($image->image_path):?>
			<div>
            	<?php 
					...
					{	?>
						<img class="imagepick" rel="<?=$image->image_path?>" src="/media/imagecache.php?image=<?=$image->image_path?>&height=150&width=150&cropratio=150:150&color=FFFFFF"/>
                	<?php
                	}
					?>
...
<? if($searchterm):?>
	...
	$(".uploaded_image").load(function (){
		...
		window.opener.$("img#<?=$regUpload?>").attr('src', '/media/imagecache.php?width=200&height=200&image='+$url);
