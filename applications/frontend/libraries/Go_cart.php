...
class go_cart {
	...
	function __construct() 
	{
		...
		$this->CI->load->model(array('Product_model'));
	...
	********************************************************/
	...
	private function _apply_coupon_to_product($cartkey, $coupon_code) 
	{
		...
		$price = (float) $this->_cart_contents['items'][$cartkey]['price'];
			...
			$discount_amount = (float) $this->_cart_contents['coupon_list'][$coupon_code]['amount']; 
	...
	private function _insert_coupon($coupon_code)
	{
		...
		$coupon = $this->CI->Product_model->get_coupon_by_code($coupon_code);
		...
		if($coupon) 
		{
			...
			if(!isset($this->_cart_contents['coupon_list'][$coupon_code]))
			{
				...
				// check validity
				if($this->CI->Product_model->is_valid($coupon)) {
					...
					} else if($coupon['type']=="Amount Off Entire Cart")
					{
					...
					{ 
						...
						foreach($this->_cart_contents['items'] as $key=>$item) {
							...
							if($item['id'] == $coupon['item_id']) { 
								...
								if($this->_apply_coupon_to_product($key, $coupon['promo_code'])) $is_applied = true;
					...
					if(!$is_applied) {
						...
						return array('message'=>'coupon_not_apply');
					} else {
						// message coupon applied
						return array('message'=>'coupon_applied');
					}
					
				} else {
					// message coupon no longer valid
					return array('error'=>'coupon_invalid');
				}
				
			} else {
				// message coupon already applied
				return array('error'=>'coupon_already_applied');
			}
		} else {
			// invalid code error message
			return array('error'=>'invalid_coupon_code');
		}	
	}
	...
	{		
		...
		if($recalculate) 
		{
			...
			foreach ($this->_cart_contents['items'] as $key => &$val)
			{
					...
					if ( $val['shippable']== 'yes')
					{
						$this->_cart_contents['order_insurable_value']  += $this_price;
						$this->_cart_contents['order_weight'] 			+= $weight*$val['quantity'];
			...
			//$this->_compute_tax();// calculate tax before shipping
			...
			// finally			
			if($this->CI->session->userdata('shippingState')!=''){			
				$this->_cart_contents['cart_total'] += $this->recalculateTax();				
			}else{
				$this->_cart_contents['cart_total'] -= $this->order_tax();
				}
...
function _remove_tax()
	...
	********************************************************/
	...
	function update_cart($qty_list=false, $coupon_code=false, $gc_code=false, $shipping=false, $tax=false)
	{
	...
	function save_order() {
		...
		$save['customer']['firstname']	= $bill['name']; 
		$save['customer']['phone']		= $bill['phone'];
		$save['customer']['email']		= $bill['email'];
		
		$save['address']['bill_address'] 	= $bill['address'];	
		$save['address']['bill_city']		= $bill['city'];
		$save['address']['bill_zip']		= $bill['zip'];
		$save['address']['bill_zone']		= $bill['state'];		
		$save['address']['bill_country_id']	= $bill['country'];
		
		
		$save['address']['ship_address'] 	= $ship['address'];	
		$save['address']['ship_city']		= $ship['city'];
		$save['address']['ship_zip']		= $ship['zip'];
		$save['address']['ship_zone']		= $ship['state'];		
		$save['address']['ship_country_id']	= $ship['country'];
		...
		$save['items']				= $this->_cart_contents['items'];
		...
		foreach ($this->_cart_contents['items'] as $item)
		{
				...
				$product_qty		= $this->CI->Product_model->getProductQty($item['upc']);
				
				$new_quantity	= intval($product_qty) - intval($item['quantity']);
				$product_quantity	= array('attribute_record_id'=>$item['upc'], 'quantity'=>$new_quantity);	
				$this->CI->Product_model->update($product_quantity);
		}
